 ...
 *				- M_MULR()			Multiplication as subfunction.
 *				- M_DIVR()			Division as subfunction.
 *				- M_PCTR()			Percentage as subfunction.
 *				- M_JSRL()			Jump to subroutine with label.
 *				- M_PWMI()			Puls width modulation on GPIO fix duty.
 *				- M_PWMR()			Puls width modulation on GPIO duty over register.
 *				- HULPx_Dump()		Disassembler for ULP program.
 * 				- HULPx_process_macros_and_load()		Same as 'process_macros_and_load()',
